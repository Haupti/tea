...
Node * build_node_2(Slice slice, NamedObject ** parent_named_objects, int parent_named_objects_count){
    ...
    for(i = slice.start; i <= slice.end; i++){
        ...
        if(temp_token.type == SET){
            int identifier_pos = i+1;
            Slice body_slice = find_assignment_body(new_slice(slice.arr, i, slice.end));
            Node * ref = build_node_2(body_slice, named_objects, named_objects_count);
    ...
    if(combinator_pos == -1){
        ...
        for(current_token_pos = after_assignments_end; current_token_pos <= slice.end; current_token_pos++){
            ...
            if(current_token.type == NOT){
                ...
                Node * node = build_node_2(grp_body_slice, named_objects, named_objects_count);
                if(nots != 0){
        ...
        Node * left = build_node_2(left_slice, named_objects, named_objects_count);
        Node * right = build_node_2(right_slice, named_objects, named_objects_count);
...
Node build_tree(Token * tokens, size_t start_token, size_t end_token){
    ...
    return *build_node_2(slice, initial_named_objects, initial_named_objects_count);
}
